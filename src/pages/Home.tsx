...
const StaticProductCard: React.FC<{ product: StaticProduct }> = ({ product }) => {
  ...
  return (
    <div className="group bg-white rounded-xl sm:rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-blue-200 sm:hover:-translate-y-2 cursor-pointer active:scale-[0.98]">
      <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-gray-100 to-gray-50">
        ...
        <div className="absolute top-2 sm:top-3 left-2 sm:left-3 flex flex-col gap-1 sm:gap-2">
          {hasDiscount && (
            <div className="bg-gradient-to-r from-red-500 to-pink-600 text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-bold shadow-lg flex items-center gap-1">
        ...
        <div className="absolute top-2 sm:top-3 right-2 sm:right-3 flex flex-col gap-1 sm:gap-2 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300">
          <button className="p-1.5 sm:p-2.5 bg-white/95 backdrop-blur-sm rounded-lg sm:rounded-xl shadow-lg hover:bg-white hover:scale-110 transition-all duration-200">
            <Heart size={14} className="text-gray-700 sm:hidden" />
            <Heart size={18} className="text-gray-700 hidden sm:block" />
          </button>
        </div>

        {/* Quick View Overlay - desktop only */}
        <div className="hidden sm:block absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-4 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
          <button
      ...
      <div className="p-3 sm:p-5">
        <div className="text-[10px] sm:text-xs text-blue-600 font-semibold mb-0.5 sm:mb-1 uppercase tracking-wider">
          {product.category}
        </div>
        <h3 className="text-xs sm:text-sm font-semibold text-gray-900 line-clamp-2 mb-2 sm:mb-3 min-h-[32px] sm:min-h-[40px] group-hover:text-blue-600 transition-colors">
          {product.name}
        </h3>
          ...
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                size={10}
                className={`sm:hidden ${star <= product.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200'}`}
              />
            ))}
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={`lg-${star}`}
                size={14}
                className={`hidden sm:block ${star <= product.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200'}`}
              />
            ))}
          </div>
        ...
        <button
          onClick={handleAddToCart}
          className="w-full flex items-center justify-center gap-1.5 sm:gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-2 sm:py-3 px-3 sm:px-4 rounded-lg sm:rounded-xl transition-all duration-200 text-xs sm:text-sm font-bold shadow-md hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98]"
        >
...
export const Home: React.FC = () => {
  ...
  // Touch swipe handling for hero carousel
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      >
        {heroSlides.map((slide, index) => (
          ...
          >
            <div className={`bg-gradient-to-br ${slide.gradient} min-h-[280px] sm:min-h-[400px] md:min-h-[500px]`}>
              {/* Decorative elements */}
              <div className="absolute inset-0 overflow-hidden">
                ...
                <div className="absolute bottom-10 right-10 w-64 sm:w-96 h-64 sm:h-96 bg-white/5 rounded-full blur-3xl"></div>
              </div>

              <div className="relative max-w-7xl mx-auto px-4 py-10 sm:py-16 md:py-24 flex items-center">
                <div className="flex-1 text-white z-10">
                  <div className={`inline-flex items-center gap-2 ${slide.accent} bg-white/10 backdrop-blur-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold mb-3 sm:mb-4 animate-pulse`}>
                    <Sparkles className="w-3 h-3 sm:w-4 sm:h-4" />
                    {slide.subtitle}
                  </div>
                  <h1 className="text-2xl sm:text-4xl md:text-6xl font-extrabold mb-2 sm:mb-4 tracking-tight">
                    {slide.title}
                  </h1>
                  <p className="text-sm sm:text-lg md:text-xl text-white/80 mb-4 sm:mb-8 max-w-lg">
                    {slide.description}
                  </p>
                  <div className="flex gap-3 sm:gap-4">
                    <Link
                      to="/products"
                      className="bg-white text-gray-900 px-5 sm:px-8 py-2.5 sm:py-4 rounded-xl font-bold hover:bg-gray-100 transition-all transform hover:scale-105 shadow-xl text-sm sm:text-base"
                    >
                      Shop Now
                    </Link>
                    <button className="border-2 border-white/30 text-white px-5 sm:px-8 py-2.5 sm:py-4 rounded-xl font-semibold hover:bg-white/10 transition-all backdrop-blur-sm text-sm sm:text-base">
                      Learn More
                    </button>
                  </div>
                </div>
        ...
        <div className="bg-gradient-to-br from-violet-600 to-indigo-700 min-h-[280px] sm:min-h-[400px] md:min-h-[500px] opacity-0 pointer-events-none"></div>
      ...
      {/* Search Bar */}
      <section className="max-w-5xl mx-auto px-4 -mt-6 sm:-mt-8 relative z-30">
        <form onSubmit={handleSearch}>
          <div className="flex bg-white rounded-2xl overflow-hidden shadow-2xl border border-gray-100">
            <div className="flex-1 flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4">
              ...
              <input
                ...
                className="flex-1 text-gray-900 focus:outline-none bg-transparent text-sm sm:text-lg min-w-0"
              />
            </div>
            <button
              type="submit"
              className="px-4 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold hover:from-blue-700 hover:to-indigo-700 transition-all text-sm sm:text-lg flex-shrink-0"
            >
              Search
            </button>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className="flex items-center gap-3 sm:gap-4 bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow"
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl flex items-center justify-center text-blue-600 flex-shrink-0">
                {feature.icon}
              </div>
              <div className="min-w-0">
                <h4 className="font-semibold text-gray-900 text-xs sm:text-sm">{feature.title}</h4>
                <p className="text-[10px] sm:text-sm text-gray-500 truncate">{feature.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-4 sm:py-6">
        <h2 className="text-lg sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Shop by Category</h2>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 sm:gap-4">
          {categories.map((cat) => (
            <button
              key={cat.name}
              onClick={() => window.location.href = `/products?category=${encodeURIComponent(cat.slug)}`}
              className="group bg-white rounded-2xl p-3 sm:p-4 shadow-sm hover:shadow-lg transition-all border border-gray-100 hover:-translate-y-1 active:scale-95"
            >
              <div className={`w-10 h-10 sm:w-14 sm:h-14 mx-auto mb-2 sm:mb-3 rounded-xl sm:rounded-2xl bg-gradient-to-br ${cat.color} flex items-center justify-center text-xl sm:text-2xl shadow-lg group-hover:scale-110 transition-transform`}>
                {cat.icon}
              </div>
              <span className="text-xs sm:text-sm font-semibold text-gray-700 group-hover:text-blue-600 transition-colors line-clamp-1">
                {cat.name}
              </span>
            </button>
          ))}
      ...
      <section className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <Link
            to="/products"
            className="flex items-center gap-1 sm:gap-2 text-blue-600 font-semibold hover:text-indigo-600 transition-colors bg-blue-50 hover:bg-blue-100 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm"
          >
            View All
      ...
      {/* Promotional Banner */}
      <section className="max-w-7xl mx-auto px-4 py-4 sm:py-6">
        <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-r from-purple-600 via-pink-600 to-red-500 p-6 sm:p-8 md:p-12">
          <div className="absolute inset-0 bg-black/10"></div>
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl"></div>
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="text-white text-center md:text-left">
              <h3 className="text-xl sm:text-3xl md:text-4xl font-extrabold mb-1 sm:mb-2">Become a Seller</h3>
              <p className="text-white/80 text-sm sm:text-lg">Start selling on BongoPortus and reach millions</p>
            </div>
            <Link
              to="/become-seller"
              className="bg-white text-purple-600 px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-bold hover:bg-gray-100 transition-all transform hover:scale-105 shadow-xl whitespace-nowrap text-sm sm:text-base"
            >
              Start Selling Today
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <Link
            ...
            className="flex items-center gap-1 sm:gap-2 text-blue-600 font-semibold hover:text-indigo-600 transition-colors text-xs sm:text-sm"
          >
            View All
            <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4" />
          </Link>
      ...
      <section className="bg-gradient-to-br from-pink-50 to-purple-50 py-6 sm:py-8 my-6 sm:my-8">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            ...
            <Link
              ...
              className="flex items-center gap-1 sm:gap-2 text-pink-600 font-semibold hover:text-purple-600 transition-colors text-xs sm:text-sm"
            >
              View All
              <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4" />
            </Link>
      ...
      <section className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <Link
            ...
            className="flex items-center gap-1 sm:gap-2 text-amber-600 font-semibold hover:text-orange-600 transition-colors text-xs sm:text-sm"
          >
            View All
            <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4" />
          </Link>
      ...
      {/* Newsletter Section */}
      <section className="bg-gradient-to-br from-gray-900 to-gray-800 py-10 sm:py-16 mt-6 sm:mt-8">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h3 className="text-2xl sm:text-3xl font-bold text-white mb-3 sm:mb-4">Stay Updated</h3>
          <p className="text-gray-400 mb-6 sm:mb-8 text-sm sm:text-base">Subscribe to our newsletter for exclusive deals</p>
          <form className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <input
              type="email"
              placeholder="Enter your email"
              className="px-5 sm:px-6 py-3 sm:py-4 rounded-xl bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 flex-1 max-w-md backdrop-blur-sm text-sm sm:text-base"
            />
            <button
              type="submit"
              className="px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg text-sm sm:text-base"
            >
              Subscribe
            </button>
          </form>
